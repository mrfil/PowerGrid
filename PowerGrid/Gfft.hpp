...
#pragma acc data copyin(pGridData[0:2*imageNumElems]) create(pGridData_d[0:2*imageNumElems]) copyout(realXformedData[0:imageNumElems],imagXformedData[0:imageNumElems])
{
...
#pragma acc host_data use_device(pGridData_d,pGridData)
	   ...
	   // Launch FFT on the GPU
			if (Nz==1) {
				...
				deinterleave_data2d<T1>(pGridData, realXformedData, imagXformedData, Nx, Ny);
			}
			else {
				...
				deinterleave_data3d<T1>(pGridData, realXformedData, imagXformedData, Nx, Ny, Nz);
			}
        }
}
