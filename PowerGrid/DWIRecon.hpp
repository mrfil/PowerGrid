...
template<typename T1>
int DWIRecon(string dataPath, uword Nx, uword Ny, uword Nz, uword L, uword niter, uword nc, bmpi::environment &env, bmpi::communicator &world)
{
    string testPath = dataPath;
    typedef complex<T1> CxT1;
    ...
    loadmat(testPath + "PMap.mat", "PMap", &PMap);
    //The navigator phase is being referenced to zero
    mpipcSENSE<T1> S_DWI(kx, ky, kz, Nx, Ny, Nz, nc, tvec, SMap, vectorise(FM), 0 - PMap, env, world);
...
template<typename T1>
int DWIRecon(string dataPath, uword Nx, uword Ny, uword Nz, uword L, uword niter, uword nc, uword nimage,
                uword nphase, uword nslab, bmpi::environment &env, bmpi::communicator &world) {
    string testPath = dataPath;
    typedef complex <T1> CxT1;
    //uword Nx =64;
    //uword Ny =64;
    ...
    //nonlinear motion correction for DWI
    loadmat(testPath + "phaseMap_" + std::to_string(nimage) + "_" + std::to_string(nslab) + "_" +
            std::to_string(nphase) + ".mat", "PMap", &PMap);
    //PMap.zeros();
    //The navigator phase is being referenced to zero
    mpipcSENSE<T1> S_DWI(kx, ky, kz, Nx, Ny, Nz, nc, tvec, SMap, vectorise(FM), 0 - PMap, env, world);
    ...
    Col <CxT1> data;
    data = 1E-7 * data;
    loadmat(testPath + "data_" + std::to_string(nimage) + "_" + std::to_string(nslab) + "_" + std::to_string(nphase) + ".mat", "data", &data);
...
int DWIRecon(string dataPath, uword Nx, uword Ny, uword Nz, uword L, uword niter, uword nc, uword nimage,
    ...
    Col <CxT1> data;
    data = 1E-9 * data;
    loadmat(testPath + "data_" + std::to_string(nimage) + "_" + std::to_string(nslab) + "_" + std::to_string(nphase) +
            ".mat", "data", &data);
