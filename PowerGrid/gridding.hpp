...
/*****************************************************************************
    ...
    File Name   [gridding.hpp]

    Synopsis    [Implementation of the forward and adjoint non-uniform Fast
    				Fourier Transform (NUFFT) on CPU and GPU via OpenACC.]
...
int
gridding_adjoint_2D(unsigned int n, parameters <T1> params, T1 beta, ReconstructionSample <T1> *__restrict sample,
...
template<typename T1>
int
gridding_adjoint_3D(unsigned int n, parameters <T1> params, T1 beta, ReconstructionSample <T1> *__restrict sample,
					const T1 *LUT, const uword sizeLUT,
					complex <T1> *gridData)
{
	int NxL, NxH;
...
template<typename T1>
int
gridding_forward_2D(unsigned int n, parameters <T1> params, const T1 *kx, const T1 *ky, T1 beta,
...
template<typename T1>
int
gridding_forward_3D(unsigned int n, parameters <T1> params, const T1 *kx, const T1 *ky, const T1 *kz, T1 beta,
					complex <T1> *__restrict sample,
					const T1 *LUT, const uword sizeLUT,
					complex <T1> *__restrict gridData)
{
...
#pragma acc enter data copyin(pGridData[0:2*gridNumElems]) create(pGridData_d[0:2*gridNumElems],pGridData_crop_d[0:2*imageNumElems],pGridData_crop_deAp[0:2*imageNumElems], outR_d[0:imageNumElems], outI_d[0:imageNumElems])
	...
	if (Nz==1) {
		gridding_adjoint_2D<T1>(n, params, beta, samples, LUT, sizeLUT,
								gridData);
	}
	else {
		gridding_adjoint_3D<T1>(n, params, beta, samples, LUT, sizeLUT,
								gridData);
	}
